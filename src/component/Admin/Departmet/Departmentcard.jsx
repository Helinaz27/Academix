...
function Departmentcard() {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        const response = await fetch("http://54.237.124.13:8000/basicapp/departments", {
          headers: {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (courcetableRef.current && !courcetableRef.current.contains(event.target)) {
        ...
        {departments.map((department) => (
          ...
          >
            ...
            <CardFooter className="pt-0">
              <Typography>{department.head ? department.head : "Department Head"}</Typography>
            </CardFooter>
          </Card>
        ))}
      </div>
